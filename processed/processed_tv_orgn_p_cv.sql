...
(
	...
	foreign key(br_day_week) REFERENCES sub_mst.day_week_mst (day_week),
	foreign key(house_num,br_date) REFERENCES processed.tv_sample_p_cv (house_num,sample_date),
	foreign key(data_agg_type) REFERENCES sub_mst.data_agg_type_mst (data_agg_type)
)
WITH (
    OIDS = FALSE
)
...
DROP VIEW IF EXISTS processed.tv_orgn_p_cv;
